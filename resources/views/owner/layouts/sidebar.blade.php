...
<aside class="app-sidebar bg-inverse-surface h-screen w-64 fixed left-0 top-0 shadow-md shadow-[#49592A]/10 flex flex-col py-8 px-4 z-50">
    <div class="mb-10 px-4">
        <h1 class="font-h2 text-h2 font-bold text-primary-fixed">TokoQ</h1>
        <p class="text-body-sm text-surface-variant/70">{{ $shop?->name ?? 'Manajemen UMKM' }}</p>
    </div>
    ...
    <nav class="flex-1 space-y-2">
        @foreach ($menuItems as $item)
            ...
            <a
                class="{{ $isActive ? 'bg-primary-container text-on-primary-container rounded-xl border-l-4 border-primary-fixed' : 'text-surface-variant hover:text-primary-fixed-dim hover:bg-surface-variant/10' }} flex items-center gap-3 px-4 py-3 transition-colors duration-200 active:scale-95 transition-transform"
                href="{{ $item['route'] }}"
            >
    ...
    <div class="mt-auto space-y-2 border-t border-surface-variant/20 pt-6">
        <button class="w-full bg-primary-fixed text-on-primary-fixed py-3 rounded-xl font-bold mb-4 active:scale-95 transition-transform" type="button">
            Upgrade Plan
        </button>
        <button class="flex w-full items-center gap-3 px-4 py-2 text-left text-surface-variant hover:text-primary-fixed-dim" type="button">
            <span class="material-symbols-outlined">help</span>
            <span class="font-body-md">Bantuan</span>
        </button>
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="flex items-center gap-3 px-4 py-2 text-error-container hover:text-on-error-container w-full text-left">
